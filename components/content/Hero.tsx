...
export default function Hero({ contentType }: HeroProps) {
	...
	const config_props = {
		blog: {
			video: config.properties.franz8495,
			title: "Our Blog",
			subtitle: "Insights & Market Updates",
			ctaButton: "Latest Articles",
			ctaTarget: "articles",
			altButton: "Subscribe",
			altTarget: "newsletter"
		},
		press: {
			video: config.properties.hwy3245,
			title: "Press & Media",
			subtitle: "Announcements & Media Coverage",
			ctaButton: "Latest Releases",
			ctaTarget: "press-releases",
			altButton: "Media Inquiries",
			altTarget: "contact"
		}
	};

	const currentConfig = config_props[contentType];
	const propertyVideo =
		"https://redesign-media.s3.eu-north-1.amazonaws.com/michaud-rauer/66ndeerhaven3.mp4";
				...
				<motion.div
					...
					<motion.div
						...
						<button
							onClick={() => {
								if (heroRef.current) {
									// If using section height, you can keep this
									const heroHeight =
									...
									// Or use scrollToSection with the target ID
									scrollToSection(currentConfig.ctaTarget);
								}
							}}
							...
							{currentConfig.ctaButton}
						</button>
						<button
							onClick={() =>
								scrollToSection(currentConfig.altTarget)
							}
