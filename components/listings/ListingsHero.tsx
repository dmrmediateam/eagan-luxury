...
interface ListingsHeroProps {
	...
	videoSrc?: string;
	thumbnailSrc?: string;
	propertyKey?: keyof typeof config.properties;
}
...
const ListingsHero: React.FC<ListingsHeroProps> = ({
	title,
	subtitle,
	videoSrc,
	thumbnailSrc,
	propertyKey = "veeder3000" // Default to 4th property
}) => {
	...
	// Get property video and thumbnail based on provided props or default
	const property = config.properties[propertyKey];
	const videoSource =
		videoSrc || (property && "vid" in property ? property.vid : "");
	const thumbnailSource =
		thumbnailSrc || (property && "thumb" in property ? property.thumb : "");
		...
		<section
			...
			<div className="relative z-10 px-[5%] lg:px-[10%] xl:px-[15%] flex flex-col items-start justify-start w-full h-full text-white">
				<motion.div
					...
					{subtitle && (
						<motion.p
							className="text-sm md:text-base font-light text-white/80 max-w-3xl mb-6"
							variants={itemVariants}>
							{subtitle}
						</motion.p>
					)}
					...
					<motion.div
						...
						<motion.button
							onClick={() => {
								...
								if (featuredSection) {
									smoothScrollTo(featuredSection.offsetTop);
								}
							}}
						...
						<motion.button
							onClick={() => {
								...
								if (contactSection) {
									...
									// If no contact section on this page, navigate to the contact page
							...
							whileTap={{ scale: 0.98 }}>
							<span className="sm:ml-2">CONTACT US</span>
						</motion.button>
					</motion.div>
				</motion.div>
			</div>
